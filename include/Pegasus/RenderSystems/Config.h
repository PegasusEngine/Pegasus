/****************************************************************************************/
/*                                                                                      */
/*                                       Pegasus                                        */
/*                                                                                      */
/****************************************************************************************/

//! \file   Config.h (RenderSystem)
//! \author Kleber Garcia
//! \date   June 23rd, 2016
//! \brief  Render system config header file. Utilize this file to compile out systems not
//!         not used by the desired app.

#ifndef RENDER_SYSTEM_CONFIG
#define RENDER_SYSTEM_CONFIG

#define RENDER_SYSTEM_CONFIG_ENABLE_GRASS 0
#define RENDER_SYSTEM_CONFIG_ENABLE_2DTERRAIN 0
#define RENDER_SYSTEM_CONFIG_ENABLE_3DTERRAIN 1
#define RENDER_SYSTEM_CONFIG_ENABLE_DEFERREDRENDERER 1


#endif