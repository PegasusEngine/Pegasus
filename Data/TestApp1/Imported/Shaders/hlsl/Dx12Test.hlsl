...
Texture2D<float4> inputTexture5[] : register(t0, space1);
...
VS_OUT vsMain(in float2 p0 : POSITION0)
{
    ...
    output.texCoords = float4(float2(p0.xy * 0.5 + 0.5) + texSamplVal.xy, 0.0, 0.0) + inputTexture5[texSamplVal.x].SampleLevel(bilinearSampler, float2(0,0), 0);
